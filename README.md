...
![version](https://img.shields.io/badge/version-4.7.1-c4cad4?style=for-the-badge&labelColor=0d0f12)
...
sensitive data exposure, security misconfiguration, exposed directory listing,
mass assignment, vulnerable components, improper input validation, SSRF (admin
`customDomain` field), XXE (B2B XML endpoint), and insecure deserialization
(JWT RS256→HS256 algorithm confusion). Recall was 100% (nothing missed); the
0.95 F1 reflects a small number of findings not mapped to the ground-truth set.
...
the method is above. Run your tool of choice against the same Juice Shop, score
it, and compare — `benchmark_compare` ranks the runs side by side by F1. An
honest, reproducible number beats a marketing table.
