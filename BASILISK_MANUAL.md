...
*The full reference for Basilisk, the AI security assistant that lives on your Linux machine.*
...
This split is why you can let Basilisk roam: it can *look* at anything instantly, and only *changing* things involves the gate.

## 1.2 The two speeds

You choose how much Basilisk asks before it acts. One setting, **Confirm every command**, flips between them:

- **Decisive (default).** When you ask Basilisk to do something, it **just does it** — runs the command, reads the output, runs the next one — no clicking through routine work. This is what makes it feel like an operator and not a chatbot.
...
When Basilisk wants approval (in confirm mode, or always for risky actions), it shows a **card** in the chat:

- **Run cards** — a proposed shell command with a **risk level** and a **Run** button. Nothing executes until you click.
- **Diff cards** — for writing or editing a file (including Basilisk editing its own source). You see a real before/after **diff**, and nothing touches disk until you click **Apply**.
...
- Any command that would **edit Basilisk's own safety guardrail**
...
**The guarantee is not "Basilisk asks every time." It's that the one category of mistake you can't undo always keeps a human in the loop, no matter the setting.**
...
| `--remove-oracle` | Remove an old Oracle install (Basilisk untouched) |
...
Basilisk is **multi-provider** — it brings no model of its own; you point it at a cloud LLM and it does the rest. You only need a key for the provider(s) you want, set in **Settings → Backends**.
...
The **default provider stack is SiliconFlow/DeepSeek primary with a Groq fallback chain** — if your active provider fails mid-request, Basilisk can fail over to the next one you've configured. Keys live only in `~/.config/kali/settings.json`, never logged and never sent anywhere but the provider's own API.

## 2.6 First launch

Launch from the app grid (look for **Basilisk**) or run `kali` in a terminal. On first run, set your provider and key in Settings, then just start talking. Hit the ⟳ next to a provider's model dropdown to fetch its live catalogue once a key is set.
...
Basilisk has a sharp, loyal, no-filler personality. It's built to be a direct operator's partner, not a cheerful assistant. You can tune it per-machine in **Settings → Persona → Custom addendum** (these tweaks survive upgrades; direct edits to the persona file get overwritten on update).

## 3.2 The status banner

While Basilisk is working, a live banner tells you *what it's doing* in plain terms — "running nmap…", "cross-checking sources…", "building the report…" — so a long operation isn't a silent spinner.

## 3.3 The Thoughts panel

When the model exposes its chain-of-thought, Basilisk tucks it into a collapsed **💭 Thoughts** expander. It's kept **out of the reply, out of text-to-speech, and out of replayed history** — there if you want to see the reasoning, invisible if you don't.

## 3.4 Chat history

Conversations are stored in a local SQLite database (`~/.local/share/kali/chats.db`). By default Basilisk is **ephemeral**: it opens a fresh chat each launch, discards empty placeholders, and bins chats idle longer than the retention window — all tunable in Settings. Your DB is backed up before every update.

---

# Part 4 — Offensive security (the bread and butter)

This is Basilisk's core. **Every tool here is propose-only or read-only** — Basilisk plans, inventories, parses, enriches, and documents. It writes no exploit code and attacks nothing on its own. Real recon/attack commands run only through the approve-before-run gate, against scope **you** set.
...
Paste raw scanner stdout and Basilisk turns it into clean structured data — hosts, ports, services, versions, endpoints, findings — for **20+ tools**: nmap (including NSE script hits), httpx, nuclei, naabu, masscan, subfinder, ffuf, feroxbuster, gobuster, katana, whatweb, wpscan, sslscan/testssl, smbmap, netexec, nikto, gitleaks, dalfox, arjun, and more. It also **strips ANSI colour codes**, so colourised pastes don't silently drop data.
...
- **Build mode:** give a simple spec (name, severity, protocol, path, matchers) and Basilisk emits a **structurally-valid** template. You supply the specifics; the scaffold guarantees the shape.
...
This is what separates a chat log from a **defensible engagement deliverable**. Every command Basilisk runs is **automatically recorded** — you don't log anything by hand.
...
Basilisk can connect to external **Model Context Protocol (MCP)** servers — the growing ecosystem of security MCP servers that wrap tools like nmap, sqlmap, ffuf, nuclei, and ZAP. Wiring one in gives Basilisk all of that server's tools without you writing a wrapper per tool.
...
- **Tools are namespaced.** A discovered tool appears as `mcp__<server>__<tool>` so it can never be confused with Basilisk's built-in tools. `mcp_tools` lists everything wired up.
- **You don't write a server.** You point Basilisk at an existing one. A server entry is a config object: `{name, command, args, env, cwd}`. For example, a Docker-packaged server is just `{"name": "pentest", "command": "docker", "args": ["run", "-i", "the-image"]}`.

## 6.2 The safety wrapper (why this is safe)

MCP is a documented remote-code-execution surface, so Basilisk treats every MCP server as **untrusted**:
...
Once enabled and configured, just ask Basilisk to use the tool ("run an nmap scan via the pentest server"). To see what's available, ask it to list MCP tools (`mcp_tools`).
...
Checks a username across public profile sites. **Important caveat Basilisk itself observes:** a hit means a *public page exists* at that handle — **not** that it's the same person. Always confirm identity separately.
...
Ask things like "how's my system doing?" or "what's eating my disk?" and Basilisk batches several of these in one go.

---

# Part 9 — Desktop control (confirm-gated)

Basilisk can drive your **actual** desktop. It auto-detects **X11 vs Wayland** and picks the right backend (xdotool/wmctrl/scrot on X11; wtype/wlrctl/grim on Wayland; prefers Spectacle/kdialog on KDE Plasma). These are *acting* tools, so they run directly in decisive mode or as cards under Confirm-every-command.
...
Ask "open Firefox and go to my router's admin page" and Basilisk chains `launch_app` / `open_url`.
...
The big one. Basilisk can run **any shell command**. In decisive mode it executes directly, reads the output, and continues; under **Confirm every command** it becomes an approve-first card; and the **catastrophic class is always force-confirmed** regardless. **Sudo is handled safely** — your password is collected in a dialog, validated, used to cache the credential for the session, and **never stored, logged, or shown to the model**.
...
Off by default; enable in Settings. When on, Basilisk remembers things across sessions — **locally, nothing leaves the box**.
...
Off by default. When on, Basilisk can **write its own Python tools** — and it can't hurt you doing it, because of the sandbox.

- **`skill_write`** — Basilisk drafts a Python tool. Before anything runs, it's **`ast`-parsed and statically screened**, then executed in a **bubblewrap jail** (read-only system, no access to your home, network off), and it **must pass its own test**. Only then do you get a card to **Apply** it. The save goes through Basilisk's own confirm dialog.
...
Basilisk can **rewrite its own source and persona**. It proposes the full new file as a **diff**; you click **Apply**. Safeguards:
...
Basilisk reads replies aloud via **Piper** (neural, nicer voice) or **espeak-ng** (always-available fallback), with **per-message play / pause / resume** controls. The Thoughts panel is never spoken.

---

# Part 15 — Quality-of-life features

Things that make Basilisk efficient and pleasant, mostly automatic:
...
- **`/panic`** — type it and Basilisk jumps straight to a batched system-health sweep with a tight summary, no chit-chat.
- **Urgency fast-path** — if you're clearly in a hurry, Basilisk drops the preamble and leads with the most likely fix.
- **Command de-dup** (opt-in) — avoids re-running an identical command it just ran.
- **Degraded-output failover** (opt-in) — if a provider starts returning garbage, Basilisk fails over to your next configured provider.
- **Cached-sudo awareness** — if sudo already has a valid cached credential this session, Basilisk won't re-prompt for the password (when that option is on).

---

# Part 16 — The background worker (optional)

A `systemd --user` service for genuinely-headless jobs: periodic system checks, memory consolidation, and skill curation. It's **fully optional** — Basilisk works identically without it, and you can skip it at install with `--no-systemd`. Use it only if you want Basilisk doing light housekeeping in the background.

---

# Part 17 — The safety model (in full)

Basilisk is built to be **decisive by default and un-catastrophic by construction**. The complete trust boundary:

1. **Two speeds, you pick.** Read-only sensing runs free. In the default decisive mode, requested actions run and Basilisk keeps going; under **Confirm every command**, every side-effecting action is a card. Either way, the floor below holds.
...
3. **Basilisk's own safety code can't be shell-stripped.** A raw `>`, `sed -i`, `tee`, or `dd of=` aimed at `kali*.py` is force-confirmed too, so the immutable guardrail can't be edited out from under the guarded path.
...
6. **Self-written code never runs in Basilisk's process.** It's `ast`-parsed, statically screened, and executed in a **bubblewrap-isolated** child (read-only system, no home, network off) that must pass its own test before it can be saved.
...
9. **Robust tool parsing** — a multi-line document or a stray character in a tool call can't silently vanish or get mistaken for chat text: it's recovered and rendered as a real diff/run card, or you're told it wasn't — so Basilisk never claims an action happened when it didn't, and never "types" a tool call into the conversation instead of running it.
...
# Part 18 — What Basilisk can NOT do

By design:

- **Destroy your system or its storage on its own.** The catastrophic class is always force-confirmed.
- **Be an always-on autonomous fleet agent.** A deliberate non-goal — Basilisk keeps you in the loop. (Want an unsupervised swarm? That's a different kind of tool.)
...
- **Active provider + keys** (Backends) — which model Basilisk uses, and the fallback chain.
...
**Sidecar safety:** every sidecar hook is null-safe — a missing or broken sidecar can never take Basilisk down. The whole `kali_ext/` tree is off by default.

---

# Part 21 — Troubleshooting & FAQ

**Basilisk won't start / GTK errors.** Re-run the installer — it installs the GTK4 + libadwaita bindings and is safe to run again. Confirm Python 3.10+.
...
**Where's my evidence?** In `~/.config/kali/evidence/` — one JSONL file per engagement plus hashed artifact files. Ask Basilisk for an `evidence_report` for a readable view.
...
*Built by The Priest. Named for the serpent whose gaze is lethal.*
